...
precompute <- function(alpha) {
  ...
  list(ihqcd = ihqcd, originalPars = originalPars , env = environment(),
       Asder1fun = Asder1fun, Phider1fun = Phider1fun)
}
...
getFQNormalizableModes <- function(mass, m) {
  ...
  fm2     <- splinefun(m$z, m$f^2)
  dfm     <- splinefun(m$z, m$df)
  dfm2    <- splinefun(m$z, m$df^2)
  factor  <- splinefun(m$z, m$f^2 + (1/mass^2) * m$df^2)
  list(fm = fm, fm2 = fm2, dfm2 = dfm2, factor = factor)
}
...
getU1NormalizableModeRaw <- function(mass = 1.0, alpha = 0) {
  ...
  y1   <- m$y1[m$time < zf]
  y2   <- m$y2[m$time < zf]
  f <- splinefun(z,exp(pre$Asder1fun(z) - pre$Phider1fun(z))* y1 * y1)
