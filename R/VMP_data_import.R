...
loadVMP_sigma <- function() {
  # This function creates vectors with the values of m, W, Q2, sigma and delta_sigma
  ...
  delta_sigma <- c(sqrt(1.1**2+6.4**2),sqrt(3.9**2+3.6**2),sqrt(2.7**2+3.0**2),sqrt(2.4**2+2.1**2),sqrt(1.8**2+1.3**2),sqrt(1.24**2+0.72**2),sqrt(0.81**2+0.33**2),sqrt(0.24**2+0.06**2))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(2.4**2+4.0**2),sqrt(2.3**2+4.3**2),sqrt(2.8**2+5.3**2),sqrt(3.2**2+5.5**2),sqrt(3.4**2+6.4**2),sqrt(3.7**2+6.9**2),sqrt(4.0**2+7.3**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(3.5**2+8.1**2),sqrt(4.4**2+8.7**2),sqrt(6.6**2+9.6**2),sqrt(8.0**2+11.0**2),sqrt(8.0**2+12.0**2),sqrt(10.0**2+15.0**2),sqrt(14.0**2+25.0**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(10.0**2+18.0**2),sqrt(13**2+17**2),sqrt(13.0**2+18**2),sqrt(19.0**2+23.0**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(2.9**2+2.4**2),sqrt(3.4**2+2.9**2),sqrt(5.1**2+4.0**2),sqrt(8.8**2+4.5**2)))
  # Getting data from Table4.csv file
  m           <- c(m,c(m_Jpsi,m_Jpsi,m_Jpsi,m_Jpsi))
  W           <- c(W,c(53.3,83.9,114.1,144.2))
  Q2          <- c(Q2,c(7.0,7.0,7.0,7.0))
  sigma       <- c(sigma,c(12.9,14.5,24.7,24.1))
  delta_sigma <- c(delta_sigma,c(sqrt(2.5**2+1.2**2),sqrt(2.5**2+1.4**2),sqrt(4.1**2+2.4**2),sqrt(6.2**2+2.5**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(0.69**2+0.31**2),sqrt(0.7**2+0.39**2),sqrt(1.0**2+0.5**2),sqrt(1.6**2+0.6**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(32**2+59**2),sqrt(24**2+49**2),sqrt(20**2+43**2),sqrt(17**2+33**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(16**2+27**2),sqrt(13**2+20**2),sqrt(9**2+17**2),sqrt(9**2+12**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(8**2+8**2),sqrt(6.4**2+6.6**2),sqrt(4.3**2+5.5**2),sqrt(3.9**2+4.2**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(3.9**2+4.2**2),sqrt(2.9**2+2.8**2),sqrt(2.2**2+2.7**2),sqrt(1.3**2+1.7**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(1.0**2+1.3**2),sqrt(0.82**2+1.11**2),sqrt(0.58**2+0.67**2),sqrt(0.42**2+0.39**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(0.31**2+0.31**2),sqrt(0.27**2+0.24**2),sqrt(0.105**2+0.101**2)))
  # Getting data from Table3.csv file
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(8.5**2+10.6**2),sqrt(6.2**2+8.3**2),sqrt(4.5**2+5.2**2),sqrt(4.0**2+4.1**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(2.7**2+2.6**2),sqrt(2.0**2+1.9**2),sqrt(1.5**2+1.2**2),sqrt(0.90**2+0.78**2)))
  delta_sigma <- c(delta_sigma,c(sqrt(0.63**2+0.56**2),sqrt(0.38**2+0.3**2),sqrt(0.24**2+0.17**2),sqrt(0.076**2+0.049**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(14**2+30**2),sqrt(16**2+26**2),sqrt(19**2+30**2),sqrt(23**2+40**2),sqrt(27**2+40**2)))
  # Getting data from Table10.csv
  m           <- c(m,c(m_rho0,m_rho0,m_rho0,m_rho0,m_rho0))
  W           <- c(W,c(48,64,80,96,114))
  Q2          <- c(Q2,c(6.6,6.6,6.6,6.6,6.6))
  sigma       <- c(sigma,c(72.6,97.2,99.3,120,115))
  delta_sigma <- c(delta_sigma,c(sqrt(4.3**2+7.8**2),sqrt(5.9**2+9.4**2),sqrt(6.2**2+7.2**2),sqrt(9**2+10**2),sqrt(10**2+10**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(2.6**2+2.4**2),sqrt(2.7**2+2.4**2),sqrt(3.5**2+3.0**2),sqrt(4.1**2+3.5**2),sqrt(3.2**2+3.1**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(0.6**2+0.6**2),sqrt(0.9**2+0.7**2),sqrt(1.0**2+0.8**2),sqrt(1.1**2+0.8**2),sqrt(1.8**2+1.4**2)))
  ...
  delta_sigma <- c(delta_sigma,c(sqrt(0.2**2+0.1**2),sqrt(0.4**2+0.2**2),sqrt(0.5**2+0.2**2),sqrt(0.6**2+0.3**2),sqrt(0.7**2+0.3**2)))
  # Getting data from Table17.csv
  m           <- c(m,c(m_phi,m_phi,m_phi,m_phi,m_phi))
  W           <- c(W,c(41,54,67,80,93))
  Q2          <- c(Q2,c(3.3,3.3,3.3,3.3,3.3))
  sigma       <- c(sigma,c(41.2,55.1,49.2,57.5,69.6))
  delta_sigma <- c(delta_sigma,c(sqrt(4.3**2+5.3**2),sqrt(5.7**2+5.4**2),sqrt(6.9**2+6.2**2),sqrt(7.4**2+6.8**2),sqrt(8.4**2+7.5**2)))
  # Getting data from Table18.csv
  m           <- c(m,c(m_phi,m_phi,m_phi,m_phi,m_phi))
  W           <- c(W,c(48,64,80,96,114))
  Q2          <- c(Q2,c(6.6,6.6,6.6,6.6,6.6))
  sigma       <- c(sigma,c(13.2,13.0,20.5,14.7,23.3))
  delta_sigma <- c(delta_sigma,c(sqrt(1.5**2+1.5**2),sqrt(1.7**2+1.5**2),sqrt(2.3**2+1.9**2),sqrt(2.4**2+1.7**2),sqrt(4.0**2+2.2**2)))
  # Getting data from Table19.csv
  m           <- c(m,c(m_phi,m_phi,m_phi,m_phi,m_phi))
  W           <- c(W,c(71,97,116,139,165))
  Q2          <- c(Q2,c(15.8,15.8,15.8,15.8,15.8))
  sigma       <- c(sigma,c(2.3,2.5,3.9,4.4,7.6))
  delta_sigma <- c(delta_sigma,c(sqrt(0.3**2+0.2**2),sqrt(0.4**2+0.2**2),sqrt(0.6**2+0.3**2),sqrt(0.8**2+0.5**2),sqrt(3.2**2+0.6**2)))

  return(data.frame(m,W,Q2,sigma,delta_sigma))
  }

loadVMP_dsigma <- function(){
  # This function creates vectors with the values of m, W, Q2, t, dsigma and delta_dsigma
  # of the data used by arxiv:1307.0009
  # Getting data from 694372 folder
  m            <- c()
  W            <- c()
  ...
  dsigma       <- c()
  delta_dsigma <- c()
  return(data.frame(m,W,Q2,t,dsigma,delta_dsigma))
}
